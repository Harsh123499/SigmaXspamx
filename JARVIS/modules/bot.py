...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        ...
        await jarvis.edit(f"[𝐒𝐈𝐆𝐌𝐀 𝐁𝐎𝐓 𝐈𝐒 𝐑𝐄𝐀𝐃𝐘 𝐓𝐎  ](https://t.me/Sigma_Byz)[𝐅𝐔𝐂𝐊 𝐇𝐀𝐓𝐄𝐑𝐒 🥀](https://t.me/sigmas_support)🤖\n» `{mp} ᴍꜱ`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"➪ __HARSH Ka Ek Beta Aur Add Ho rha hai..__")
        target = ""
        ...
        try:
            ...
            await ok.edit("➪ 𝐂𝐇𝐔𝐓𝐈𝐘𝐀...𝐑𝐄𝐏𝐋𝐘 𝐊𝐀𝐑𝐊𝐄 𝐊𝐀𝐑 !!")
            return

        if str(target) in sudousers:
            await ok.edit(f"YE BHI HARSH KA HI BETA HAI.. !!")
        else:
            if len(sudousers) > 0:
        ...
        await event.reply("☞︎︎︎ BSDK SIRF Harsh SUDO DE SKTA HAI...")
...
async def removesudo(event):
    if event.sender_id == OWNER_ID:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        sudousers = getenv("SUDO_USERS", default=None)
        ok = await event.reply(f"YE Harsh Ki Najayaz Aulad thi isiliye nikal diya💋...")
        target = ""
        if HEROKU_APP_NAME is not None:
            app = Heroku.app(HEROKU_APP_NAME)
        else:
        ...
        await event.reply("Only Harsh can remove sudo users.")
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        sudo_users_list = "Harsh Ke Bache hai ye:\n"
...
async def addmultisudo(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.reply("Only Harsh can add sudo users.")
